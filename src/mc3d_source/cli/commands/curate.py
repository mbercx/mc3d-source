...
def main(
    ...
    query.append(
        ...
        CifCleanWorkChain, with_group='group', tag='cif_clean',
        project='attributes.exit_status'
    ).append(
        orm.CifData, with_outgoing='cif_clean',
        project=('attributes.source')
    ).append(
        orm.CifData, with_incoming='cif_clean',
        project='attributes.spacegroup_numbers'
    ).append(
        orm.StructureData, with_incoming='cif_clean',
        project='*'
    )
    ...
    with query.backend.transaction():
        ...
        for structure, extras_to_set, curated in track(
            data,
