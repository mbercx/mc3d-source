...
def main(
    ...
    target_source_ids = set(
        orm.QueryBuilder().append(
            orm.Group, filters={'label': target_group}, tag='group'
        ).append(
            orm.StructureData, with_group='group', project='extras.source.id'
        ).all(flat=True)
    )
    if len(target_source_ids) > 1:
        print(f"[bold yellow]Report:[/] Found {len(target_source_ids)} `StructureData` in target group.")
        print(f"[bold blue]Info:[/] These will be skipped in the update process.")

    target_group = orm.load_group(target_group)

    query = orm.QueryBuilder()
    query.append(
        orm.Group, filters={'label': old_group}, tag='group'
    ).append(
        orm.StructureData, with_group='group',
        project=('extras.source.id', '*'), 
    )
    old_id_to_nodes = {source_id: structure for source_id, structure in query.all()}

    query = orm.QueryBuilder()
    query.append(
        orm.Group, filters={'label': new_group}, tag='group'
    ).append(
        orm.StructureData, with_group='group',
        project=('extras.source.id', '*'), 
    )
    new_id_to_nodes = {source_id: structure for source_id, structure in query.all()}
