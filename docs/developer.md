...
Releases of `mc3d-source` are cut by pushing a `vX.Y.Z` tag to GitHub.
...
1. Bump the version and generate the changelog draft:
    ...
    This runs `hatch version` to update `src/mc3d-source/__about__.py`, then runs `dev/update_changelog.py` to prepend a new section to `CHANGELOG.md` with commits sorted by type.
